function getAllHeadingFontStyles() {
    ...
    document.querySelectorAll('*').forEach(element => {
        ...
        const styleIdentifier = `${fontSize} - ${fontFamily} - ${lineHeight} - ${letterSpacing} - ${fontWeight} - ${fontStyle}`;
    ...
    return Array.from(fontStyles).map(style => {
        ...
        const [fontSize, fontFamily, lineHeight, letterSpacing, fontWeight, fontStyle] = style.split(' - ');
        return { fontSize, fontFamily, lineHeight, letterSpacing, fontWeight, fontStyle };
    });
}
